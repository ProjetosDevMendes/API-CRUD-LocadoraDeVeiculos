...
app.get("/VerificarCliente/:telefone", async (req, res) => {
  try {
    ...
    async function verificarClientePorTelefone(telefone) {
      const connection = await mysql.createConnection(dbconfig);
      const [results] = await connection.execute(
        "SELECT nome, telefone, email, cpf FROM Cliente WHERE telefone = ?",
        [telefone]
      );
      connection.end();
      return results;
    ...
    if (clienteEncontrado.length > 0) {
      ...
      const cpf = primeiroCliente.cpf;
      const email = primeiroCliente.email;

      res.status(200).json({ nome, cpf, email, telefone });
    } else {
...
app.get("/Cliente/:cpf", async (req, res) => {
  try {
    ...
    if (informacoesCliente.length > 0) {
      ...
      res.status(200).json({ mensagem: "Acesso permitido", nome: nomeCliente });
    } else {
      res
        .status(404)
